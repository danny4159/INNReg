...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    elif netG == 'INN_original':
        net = InvISPNet(input_nc, output_nc,subnet_name='DBNet', block_num=8)
        # net = InvISPNet(channel_in=3, channel_out=3, block_num=8)
    elif netG == 'INN_attention':
        net = InvISPNet(input_nc, output_nc, subnet_name='DWBlock', block_num=8)
    elif netG == 'INN_DWDB':
        net = InvISPNet(input_nc, output_nc, subnet_name='DWDB', block_num=8)
...
def define_Dec(input_nc, output_nc, ngf, norm='batch', use_dropout=False, init_type='normal',
             init_gain=0.02, no_antialias=False, gpu_ids=[]):
    norm_layer = get_norm_layer(norm_type=norm)
    net = ResnetDecoder(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=4, no_antialias=no_antialias)
...
def define_Projector(input_nc, output_nc, init_type='normal',
             init_gain=0.02, gpu_ids=[]):
    class Projector(nn.Module):
        def __init__(self, input_nc, output_nc):
            super(Projector, self).__init__()
            self.avgpool = torch.nn.AdaptiveAvgPool2d((1, 1))
            self.linear1 = torch.nn.Linear(input_nc, input_nc)
            self.relu1 = torch.nn.ReLU()
            self.linear2 = torch.nn.Linear(input_nc, output_nc)
            self._init_weight()
    ...
    net = Projector(input_nc, output_nc)
    return net
